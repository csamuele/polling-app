...
const MyVotes: React.FC = () => {
    ...
    const { data: votes, error, isLoading } = useQuery(
        $api.queryOptions('get', '/api/user-votes/',)
    )
    const queryClient = useQueryClient();

    const deleteVoteMutation = useMutation({
        mutationFn: (id: number) => fetchClient.DELETE("/api/votes/{id}/", { 
            params: { path: {id}},
        }),
        onSuccess: () => {
    ...
    return (
        <VoteContextProvider>
            <div className="container mx-auto p-4">
                ...
                {votes && <VoteGrid votes={votes} isLoading={isLoading} error={error} canEdit onDelete={handleDelete}/>}
            </div>
        </VoteContextProvider>
    );
};
